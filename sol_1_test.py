...
def build_kd_tree(monsters):
    if not monsters:
        return None, []
    positions = np.array([(monster["x"], monster["y"]) for monster in monsters])
        ...
        while turns < max_turns:
            ...
            if target:
                ...
                if monsters_alive[target - 1]["hp"] <= 0:
                    ...
                    kd_tree, positions = build_kd_tree(monsters_alive)  # Rebuild KDTree
                    closest_idx = 0
                    target = 0
            else:
                ...
                if target:
                    ...
                    if monsters_alive[target - 1]["hp"] <= 0:
                        ...
                        score += monsters_alive[target - 1]["gold"]
                        monsters_alive.pop(target - 1)
                        kd_tree, positions = build_kd_tree(
                            monsters_alive
                        )  # Rebuild KDTree
                        closest_idx = 0
                        target = 0
